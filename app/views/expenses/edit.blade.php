        ...
        <div class="form-group">
            ...
            <select name="from_account" class="form-control" required>
              ...
              @foreach($from_ac as $from_ac)
                <option value="{{$from_ac->id }}"<?= ($expense->account_from==$from_ac->id)?'selected="selected"':''; ?>> {{ $from_ac->name }}</option>
              @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="to_account" class="form-control" required>
              ...
              @foreach($to_ac as $to_ac)
                <option value="{{$to_ac->id }}"<?= ($expense->account_to==$to_ac->id)?'selected="selected"':''; ?>> {{ $to_ac->name }}</option>
              @endforeach
            </select>
        </div>
